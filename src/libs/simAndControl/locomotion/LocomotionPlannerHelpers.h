...
private:
    ...
    void generate(const bFrameState& startingbFrameState) {
        ...
        while (t < tEnd) {
            ...
            // position = pos + vForwardVector * dt + vSidewaysVector * dt, where *Vector is expressed in world coordinates based on headingAngle, not in body frame.

            V3D vForwardVector = vForward * V3D(heading * robot->forward);
            V3D vSidewaysVector = vSideways * V3D(heading * RBGlobals::worldUp.cross(robot->forward));

            pos = pos + vForwardVector * dt + vSidewaysVector * dt;

            //std::cout<<" Quaternion heading is " << heading.w()<< " " << heading.x() << " " << heading.y() << " " << heading.z()<< std::endl;
